...
Two catalog tables are used to manage the state of the TerrascaleFS file namespace. The main table, called the **inode table**, is a table of files consisting of each file's inode number, number of active references, URI to the underlying block or object storage location for this file, the active/sealed state, and the node which currently owns this file (for active files). For sealed files which store a durable block index (as will be discussed in the Block Addressing section below), the row also stores the byte offset range of that index within the object-store object, allowing callers to efficiently retrieve the index before fetching individual blocks.
