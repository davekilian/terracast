...
Each TerrascaleFS file is encrypted with a unique encryption key. This encryption key is encrypted using the cluster-wide key-encryption key (KEK), and the result is then stored directly in the file, in a TerrascaleFS-internal header. When a file is opened, the TerrascaleFS library reads the key from the header, decrypts it using the cluster KEK, and use that key for block-level encryption and decryption. Each block is individually encrypted using the AES256 cipher in Galois Counter Mode (GCM). The block's position within the file is used as the initialization vector; that is, the first block has an IV of 0, the second has an IV of 1, and so on. Since each block has a unique position within the file, these positions serve as nonces.

This scheme provides privacy by encrypting all file data. Because each block has a unique (encryption key, IV) pair, no two blocks have the same ciphertext, even if those two blocks have the same plaintext. That each block has a unique (key, IV) pair also prevents tampering via cipher reuse attacks: if an attacker copies the ciphertext for a block into another file, the keys will not match; if the attacker copies the ciphertext for a block elsewhere in the same file, the new copy will have a different block position within the file, so the IVs will not match.
