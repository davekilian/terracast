...
Dual-indexing enables hybrid transactional/analytical processing, at the cost of roughly doubling the write- and storage-amplification overhead compared to a row-only or column-only index. Storing (relatively large) row payloads out-of-page in a separate, garbage-collected row store as suggested by WiscKey reduces the write- and space-amplification overhead of dual-indexing to a manageable level.

In addition to regular scalar primary key indexing, TerrascaleDB supports range types optimized for spatial and temporal data. Overlapping ranges are automatically broken down at indexing time. The columnar index supports efficient lookup of objects which intersect an n-dimensional bounding box created by specifying one range along each axis. For geospatial and temporal queries, the query layer (described later in this document) provides further refinement involving fine-grained collision detection and spatial joins.
