...
TerrascaleFS automatically tiers files by storing active files in node-local, high-cost, low-latency block storage, and offloading sealed files to cluster-wide low-cost, high-latency object storage. This provides a single-writer mutable model for the small amount of data a cluster is currently working on, anda multi-writer immutable model for most data the cluster needs. Storing data immutably in object storage reduces cost and makes it simple to cache instances of data on any node that needs it during a cluster-wide parallel processing step.

TODO the exact set of steps that need to happen? First the active model is marked sealed in the inode table, then a cleaned copy is written out to volatile block storage, with the block headers and logical-physical mapping stored at the end of the file, with flush buffer headers and empty flush buffer space removed. An object name is generated and stored in the catalog. Then the corresponding object is created and filled with buffer we just created on disk. Finally, the item is marked visible in the inode table, and the block URI is removed. Lazily, the old durable copy and the cleaned volatile copy of the file are removed from the node's local memory.

Mention that we could consider doing all this inline so no extra copy has to be written. It's possible in principle but the memory overhead is higher.

DOPPEL-TODO: do we need a logical-physical index and a list of block headers, or doesn't a list of block headers suffice? Like a B+ tree of logical indexes bottomming out in block header arrays in the leaf pages?
