...
Two catalog tables are used to manage the state of the TerrascaleFS file namespace. The main table, called the **inode table**, is a table of files consisting of each file's inode number, number of active references, URI to the underlying block or object storage location for this file, the active/sealed state, and the node which currently owns this file (for active files). Files are created by inserting rows into the inode table with a reference count of 1, and each time a file is unlinked, its reference count is decreased; when the reference count falls to zero, the file is deleted.

A second **inode number table** tracks unused inode number ranges, allowing TerrascalefS to efficiently allocate and recycle inode numbers in a peer-to-peer manner. Each row of this table is represents a contiguous range of available inodes. To allocate an inode, a node chooses a row at random, takes the lowest inode from that row, and does a batch transaction which deletes the original range and inserts a new row for the unallocated remainder of the range. For example, if the allocator picks the row (100, 200), it might allocate inode number 100, delete the (100, 200) row and insert a (101, 200) row in its place. When a file is deleted, its inode number is recycled back into this table, by merging it with the neighboring range(s) if possible, or adding it as a standalone row otherwise.

### File Refcounts

Each file has a durable **reference count** which is stored in the inode table. When a caller needs long-term access to a file, it places a reference on that file. When the file is no longer needed, the caller 'unlinks' the file, removing its reference. Files are created with a reference count of 1, and if the reference count of the file ever drops to zero, it is deleted &mdash; its contents are deleted from external block/object storage, and its row is then removed from the table. When a file's reference count reaches zero, it is 'logically deleted' insofar as it does not appear in file table enumeration calls and new references to the file are no longer permitted.

File reference counts are useful in the situation where a file has multiple distinct users, and the users must all coordinate to determine when it is safe to delete the file. For example, TerrascaleDB publishes a single checkpoint file containing LSM Tree data down to object storage, with a single reference per shard of the next-level LSM Tree into which the file needs to be merged. Each team the relevant data of the checkpoint is merged into the underlying LSM Tree shard, a reference is released on the checkpoint's file. Thus, once the checkpoint has been completely merged, its refcount drops to zero, allowing it to be cleaned up.

It is possible for nodes to fail after a file has been logically deleted (i.e. when its refcount drops to zero) but before the contents have been deleted from external storage and/or the catalog row entry has been removed. A periodic **fsck** process scans the inode table for files that have zero references remaining and completes the process of deleting them from external object storage, returning the inode number to the inode number table, and finally deleting the file from the inode table. Because, in rare cases, the fsck process can delete a file that another node is actively deleting in parallel, all steps of deleting a file must be idempotent.

### Block Addressing

TerrascaleFS files expose an API interface where a single file appears to be a byte array, consisting of the concatenated contents of each block that was appended to that file. These logical addresses do not map trivially to byte offsets on disk: each block is compressed, meaning the actual size of the block on disk is not the same as its logical size as seen by the caller, and second, the file system needs to manage block-level metadata including checksums, initialization vectors for encryption, and so on, which are present in the file but not the caller's logical view of the file.

For each block in a file, Terrascale stores a **block header** on disk consisting of block-level metadata, including

* The logical address and size of the block (the byte offset and size that API callers see)
* The physical address and size of the block on disk (the byte offset and size used to store and retrieve individual blocks)
* A logical data checksum of the original block contents before the data was compressed and encrypted
* A physical data checksum of the block contents as they appear on disk, after compression and encryption

For each file it has open, TerrascaleFS caches in the client's memory a chunked array of block headers of all blocks that exist in that file. For large files consisting of many blocks, the block headers are further indexed using a B+ tree indexed by block logical addresses for fast lookup. 

Active files are stored as a single sequential log of blocks, where each block consists of a block header followed immediately by the postprocessed (compressed and encrypted) block contents. Storing the header inline with the data ensures that one logical client append can be satisfied with a single physical append to disk. The block header index for an active file is rebuilt in memory on demand each time the file is read or written: for a read to a certain position, or for any append to the end of the file, the file system internally reads all its cached block headers, and then continues reading block headers on a time from disk, until finally reaching the point in the file the caller is trying to read/append. This is effectively an on-demand one-time scan of the full file, which is acceptable because active files are rarely reopened other than in fault recovery paths, where the user is likely to need to read the entire file anyways. (Write-ahead logs work this way, for example.)

Sealed files are stored using the same format, except that block headers are no longer stored inline with block contents; instead, block contents occur contiguously in one region of the underlying file, and the block header array is stored in a separate region. The B+ tree mapping logical addresses to chunks of the block header array is also stored in the same region as the block headers themselves. The byte offset and length of the region which stores the block headers and the index over those block headers is stored in the inode table, allowing the system to efficiently load and cache the block header index for a file when the file is opened, without needing to scan the whole file. Sealed files are immutable, and often many times larger than the average active file, so storing the block header index separately helps accelerate demand-paging read patterns.

### Buffering Appends

Solid state flash devices internally use a log-structured format, where small pages of data are written into larger erase blocks. Because data can only be erased at the erase-block level, pages which are updated in-place by the caller must be rewritten entirely on flash using a full read-modify-write cycle. This introduces write latency/throughput overhead and, in aggregate, causes flash memory devices to wear out faster.

To address these limitations of flash storage, TerrascaleFS buffers small writes in memory until a full page of data is available, and then flushes entire page-aligned runs of data to disk. Flush buffers do not need to be aligned with file blocks; if a block straddles two more buffers, the block may be flushed in stages, as each flush buffer is individually filled. Each flush buffer is written to disk with its own header, which is read when the file is loaded. If, upon opening a file, it is found that a block was only written partially (it straddles flush buffers and only some of those flush buffers exist on disk), the entire block is ignored, as if no part of it were ever flushed to disk at all.

The caller can request that file data be flushed durably or in best-effort; in the former case, the I/O call is held until all underlying flush buffers have been stored durably on disk; in the latter case, the I/O call completes as soon as the block has been buffered in memory. To avoid holding an I/O operation for an excessively long time should there not be enough data to completely fill a flush buffer, a timer is started for each flush buffer upon allocation, and when the timer elapses, the entire flush buffer is written to disk, whether or not it is full. Because every buffer is aligned to the size of an SSD page, this results in wasted space within the file; that space is reclaimed later when the file is sealed (see Sealing and Publishing below). Flush buffers are force-zeroed with a routine such as `explicit_bzero` to ensure unused flush buffer space does not leak the previous contents of memory.

In some situations, callers may still want to build their own buffeiring scheme for small writes, as each write sent to the TerrascaleFS layer will result in a block header being appended to the file; for callers with very small write sizes (such as write-ahead logging records), the caller may want to buffer at its own level before submitting batches of records to TerrascaleFS. This reduces space overhead by reducing the number of block headers TerrascaleFS must write to dissk.

### Encryption

All block and object store endpoints used by a Terrascale cluster are secured using secrets maintained withing the Terrascale cloud orchestrator. To protect user data in the event one of these endpoints is misconfigured or compromised, TerrascaleFS additionally encrypts all file contents, providing privacy even for compromised files as well as protecting from attempts to tamper with the file.

Each TerrascaleFS file is encrypted with a unique encryption key. This encryption key is encrypted using the cluster-wide key-encryption key (KEK), and the result is then stored directly in the file, in a TerrascaleFS-internal header. When a file is opened, the TerrascaleFS library reads the key from the header, decrypts it using the cluster KEK, and use that key for block-level encryption and decryption. Each block is individually encrypted using the AES256 cipher in Galois Counter Mode (GCM). For the initialization vector, TerrascaleFS concatenates two 4-byte (32-bit) fields:

* The logical block number (LBN) for this block within the file. The LBN is 0 for the file's first block, 1 for the second block, and so on. Each block has a unique LBN, even for zero-length blocks
* The flush buffer number (FBN) of the flush buffer which contained the block's block header. If a block is incompletely written to disk before the caller crashes and reopens the file, a new block with the same LBN may be rewritten later; however, the new instance of the block will have a different FBN, which makes the (FBN, LBN) pair unique for each block within the file.

This scheme provides privacy by encrypting all file data. Because each block has a unique (encryption key, IV) pair, no two blocks have the same ciphertext, even if those two blocks have the same plaintext. That each block has a unique (key, IV) pair also prevents tampering via cipher reuse attacks: if an attacker copies the ciphertext for a block into another file, the keys will not match; if the attacker copies the ciphertext for a block elsewhere in the same file, the new copy will have a different LBN, FBN, or both, meaning the IV will not match.
...
1. First, the postprocessed (compressed and encrypted) contents of each block are concatenated and uploaded to the object
2. Second, the block header array in memory is appended to the object
3. Finally, a new B+ tree which indexes the block header array by logical block address is appended to the object

Once this file is completed, the inode table row is updated to (a) mark the file as publicly visible to all nodes in the cluster, and (b) store the range of the object which contains the block header array and associated B+ tree (steps 2-3 above). At this point, the file is published: any node can open the file, by querying its object store URI and block header index byte region, download the block header index, and then read blocks on demand as needed. If any point in the process fails after the file is marked sealed but before the file has been published, then this process will restart the next time the sealed file is opened.
...
TerrascaleDB uses a two-layer model which is inspired by Apache Druid, but arranged around a log-structured merge process instead of time series. Recent writes are stored in a small, hash-partitioned "real-time" index optimized for high read/write concurrency. Data from the real-time layer is asynchronously offloaded to range-partitioned historical layer, which stores large amounts of immutable data indexed in multiple representations: a traditional row-oriented index as well as a Druid-like inverted columnar index. Queries consult both layers simultaneously. Data travels from the realtime layer to the historical layer through TerrascaleFS files in external cloud object stores &mdash; a strategy Druid refers to as "deep storage."

The TerrascaleDB **realtime** layer serves $L_0$ of the log-structured merge process. Tables are partitioned over a cluster's realtime nodes using a consistent hashing scheme based on hashing the table's primary key. Inside a realtime partition, data is stored in an ordered tree to facilitate efficient range queries locally; hashing is only used to distribute keys across realtime partitions. A multiversioning and distributed transaction model is used to implement range-based queries which cross partitions with serializable snapshot isolation. The realtime layer's hash-partitioning scheme s relatively easy to scale elastically, and helps distribute hot key ranges across all nodes evenly; however, it destroys range locality at the partition level, requiring range reads to be distributed across all realtime partitions simultaneously. This, in turn, is mitigated by using an ordered index inside each partition, as well as by limiting the overall size of the realtime database layer.

The TerrascaleDB **historical** layer serves $L_2$ of the log-structured merge process. Tables in this layer are partitioned by range, where partitions are split and merged based on size heuristics; note that the historical layer is partitioned independenly of the realtime layer.  At the historical layer, each row is indexed twice, in two different representations: one is a traditional row-oriented index, which maps primary keys to a row, and the other is an inverted columnar index, which maps unique column values to bitsets of rows, where the bitset has a $1$ for each row that has the given value in the given column. Precomputed aggregates are stored in both index representations to facilitate fast analytical queries and avoid large scans when possible. All data stored in the historical layer is immutable.

Data travels between the two layers via **checkpoints**, which serve as $L_1$ of the log-structured merge process. Checkpoints act as a buffer, allowing the realtime layer to offload memory-resident data to disk while the same data is asynchronously ingested into the historical layer. When a realtime partition reaches a size limit, it is converted to a row-oriented B+ tree, called a *checkpoint*, which maps primary keys to rows. The node can then offload the in-memory representation of the rows, while continuing to serve them through B+ tree pages read in selectively through a cache. Asynchronously, historical nodes learn of new checkpoints when they poll the database catalog, and once enough writes are staged in checkpoints, each historical partition is fully rewritten, incorporating changes from all staged checkpoints. Each checkpoint iss deleted once it has been merged into all relevant historical partitions. In this manner, data passes from an $L_0$ realtime partition, to an $L_1$ checkpoint, to $L_2$ row-oriented and columnar historical partitions.

Raw TerrascaleDB rows are serialized sequentially to a **row store**, which maps 64-bit identifiers to individual rows on the scale of a single database. The row store is the central repository for row data across the entire database: all row-oriented and columnar indexes store row identifiers of the relevant row in the external row store, instead of storing the original row in-page as part of the tree itself. This can lead to read amplification even for the primary index, since rows which are adjacent in the index may not be adjacent in the row store; however, as noted by WiscKey, modern flash storage is very good at serving highly parallel random reads, and removing rows from the index leads to much better write- and space-amortization. Storing all rows in a central repository also simplifies the task of maintaining consistency in TerrascaleDB's dual-layer realtime/historical index. A copy-forward garbage collector asynchronously cleans up rows that are no longer referenced in the realtime or historical indexes.

> So far we've only talked about the primary index. How should secondary indexes work? Note that, in this architecture, a primary index is a lot like a  traditional secondary index, so what we need is almost like having extra primary indexes. However, it's not really feasible to hash-partition each index. More likely, a secondary index has to reindex the same data inside a hash partition hashed by primary key.

### Distributed Queries

SQL queries, which consult both the realtime and historical layers of the database simultaneously, are distributed across the cluster's realtime nodes via a client-side random selection process. A Paxos-based distributed transaction model provides serializable snapshot isolation across realtime partitions; no isolation is needed at the historical layer, because all historical data is immutable. Queries contact the realtime and historical layers separately, and combine the results as needed:

* Point inserts are directed to the relevant realtime partition
* Bulk inserts are distributed across all realtime partitions using a distributed transaction model
* Point reads by primary key are directed to the relevant realtime partition and the relevant row-oriented historical partition
* Other point reads and all window reads are broadcasted to all realtime partitions, as well as relevant row-oriented historical partitions
* Aggregate reads scan over all realtime partitions as well as the optimal historical partition (usually columnar)
* All updates are broadcasted across realtime partitions, which query historical partitions internally as needed

Incremental "real-time" queries (which are read-only) are supported by setting up routing rules across all realtime partitions. A realtime query running on a single realtime node registers for updates from all other realtime nodes by broadcasting a selection filter to its peers using Paxos, and as part of the same distributed transaction, requests all results which were visible at the time the filter was installed. The query coordinator combines these realtime-layer results with the historical-layer results like normal. Meanwhile, the peer realtime nodes buffer all inserts, updates and deletes matching the filter, and periodically deliver them to the query corodinator which requested them. The query coordinator merges these batches of updates into its existing results similar to how it merges realtime-layer results into historical-layer results. The differential results are then streamed to the client who requested the incremental query. Incremental queries end with the client explicitly ends them, or when the realtime node crashes. Incremental queries cannot restart from a checkpoint when the query node crashes, but they can be restarted from scratch to produce the same results.

Distributed joins are pushed down to the historical layer, and are implemented using a Volcano-like exchange operator which dynamically repartitions data across nodes and runs single-node join algorithms internally on each node in parallel. Where possible, joins exploit existing partitions instead of dynamically repartitioning. A distributed snapshot of all realtime-layer partitions is broadcasted to all historical nodes involved in a distributed join, in order to complete their view of the world; broadcasting is acceptable because realtime partitions are very small compared to historical partitions. 

Spatial joins and other queries involving collision detection on vector data are also pushed down to the historical layer. Spatial queries run in two passes: in the first pass, the query consults the database index to identify candidate results using an axis-aligned bounding box. Bounding-box queries are in turn implemented using the columnar index, by identifying all rows which intersect each axis of the bounding box individually, then computing the set intersection of the results of each axis query to obtain the set of objects which intersect the bounding box. In the second pass of a spatial query, the query engine then performs fine-grained collision detection between candidate objects using traditional mathematical techniques. In situations like joins, where a set of geometric objects may be repeatedly queried in an inner loop, the engine may elect to create a temporary spatial subdivision structure such as an octree or kd-tree over its result set.
